...
_A_CRUISE_MIN_V_FOLLOWING = [-5.0, -4.5, -4.0, -3.0, -2.0]
_A_CRUISE_MIN_V = [-2.0, -1.5, -1.0, -0.7, -0.5]
_A_CRUISE_MIN_BP = [0.0, 5.0, 10.0, 20.0, 55.0]

# need fast accel at very low speed for stop and go
# make sure these accelerations are smaller than mpc limits
_A_CRUISE_MAX_V = [1.0, 1.0, 0.9, 0.5, 0.3]
_A_CRUISE_MAX_V_ECO = [0.8, 0.8, 0.7, 0.4, 0.3]
_A_CRUISE_MAX_V_SPORT = [1.2, 1.2, 1.1, .7, .5]
_A_CRUISE_MAX_V_FOLLOWING = [1.0, 1.0, 0.9, 0.5, 0.3]
...
class Planner():
  ...
  def update(self, sm, pm, CP, VM, PP, arne_sm):
    ...
    if enabled and not self.first_loop and not sm['carState'].brakePressed and not sm['carState'].gasPressed and not sm['carState'].regenPressed:
